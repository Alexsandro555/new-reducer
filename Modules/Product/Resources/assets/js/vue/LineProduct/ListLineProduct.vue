    ...
    <v-layout wrap row>
      <v-flex xs12 class="text-xs-center">
        <v-card>
            ...
            <v-data-table :headers="headers"
              ...
              <template slot="items" slot-scope="props">
                ...
                <td class="justify-center layout px-0">
                  ...
                  <v-btn icon class="mx-0" @click="$router.push({name: 'edit-line-product', params: {id: props.item.id.toString()}})">
          ...
          <v-card-actions>
            <v-btn @click="addLineProduct" color="primary" dark class="text-left mb-2">
              <v-icon>add</v-icon>
            </v-btn>
          </v-card-actions>
        </v-card>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState('LineProduct', ['items', 'loading']),
    },
    ...
    methods: {
      addLineProduct() {
        this.add({'title': 'По-умолчанию'})
          .then(response => {
            this.$router.push({name: 'edit-line-product', params: {id: response.id.toString()}})
          })
          .catch(error => {
          })
      },
