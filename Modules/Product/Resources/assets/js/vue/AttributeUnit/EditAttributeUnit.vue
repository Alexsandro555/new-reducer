...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState('attribute_units', ['items', 'fields']),
      ...mapGetters('attribute_units', {getItem: GLOBAL.GET_ITEM, getModel: 'getModel'}),
      form() {
    ...
    methods: {
      ...
      init(id) {
        ...
        if (this.items.length == 0) {
          this.$router.push({name: 'product-attributes-unit-list'})
        }
      },
      onSubmit() {
