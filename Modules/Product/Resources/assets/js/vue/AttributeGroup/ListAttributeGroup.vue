        ...
        <v-layout wrap row>
            <v-flex xs12 class="text-xs-center">
                <v-card>
                        ...
                        <v-data-table :headers="headers"
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class="justify-center layout px-0">
                                    ...
                                    <v-btn icon class="mx-0" @click="$router.push({name: 'edit-attributes-group-product', params: {id: props.item.id.toString()}})">
                    ...
                    <v-card-actions>
                        <v-btn @click="addAttributeGroup" color="primary" dark class="text-left mb-2">
                            <v-icon>add</v-icon>
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('AttributeGroup', ['items', 'loading']),
        },
        methods: {
            addAttributeGroup() {
                this.add({'title': 'По-умолчанию'}).then(response => {
                    this.$router.push({name: 'edit-attributes-group-product', params: { id: response.id.toString()}})
                })
            },
            goToPage(item) {
                let url = '/attributegroup/detail/'
            ...
            ...mapActions('AttributeGroup',{load: GLOBAL.LOAD, add: GLOBAL.ADD, delete: GLOBAL.DELETE, initialization: GLOBAL.INITIALIZATION})
        }
     }
</script>