  ...
  <v-container>
    <v-layout wrap row>
      ...
      <v-flex v-if="!loader">
        <v-card>
          <v-card-title>
            <h1>Редактирование группы атрибута</h1>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-layout row wrap>
                <v-flex>
                  <v-form ref="form" lazy-validation v-model="valid">
                    <template v-for="(field, num) in fields">
                      <form-builder :field="field" v-if="num!=='description'" :num="num" :items="form"
    ...
    methods: {
      ...
      init(id) {
        ...
        if (this.items.length == 0) {
          this.$router.push({name: 'product-attributes-group-list'})
        }
      },
      onSubmit() {
