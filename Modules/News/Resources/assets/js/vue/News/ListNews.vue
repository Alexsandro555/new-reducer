    ...
    <v-layout wrap row>
      <v-flex xs12 class="text-xs-center">
        <v-card>
            ...
            <v-data-table :headers="headers"
              ...
              <template slot="items" slot-scope="props">
                ...
                <td class="justify-center layout px-0">
                  ...
                  <v-btn icon class="mx-0" @click="$router.push('/news/edit/'+props.item.id)">
          ...
          <v-card-actions>
            <v-btn @click="addNews" color="primary" dark class="text-left mb-2">
              <v-icon>add</v-icon>
            </v-btn>
          </v-card-actions>
        </v-card>
    ...
    methods: {
      addNews() {
        this.add({'title': 'По-умолчанию'})
          .then(response => {
            this.$router.push({name: 'edit-news', params: {id: response.id.toString()}})
          })
      },
      goToPage(item) {
        let url = '/news/detail/'
